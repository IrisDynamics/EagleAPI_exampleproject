...
{
	///If Eagle controller is the highest COM port in device manager keep as 1 if there are 
	///other connected devices with higher COM numbers decrement accordingly
	public static int EaglePort = 1;		
	/// <summary>
	...
	public static void Write (string message)
	{
		if (checkOpen ()){
			...
			EagleAPI.lastSent = message;
		}
		else{
			EagleAPI.correctPort = false;
		}
	...
	public static string GetPortName ()
	{
		...
		if (portNames.Count > 0)
			return portNames [portNames.Count - EaglePort];
		else
			return "";
	}
